...
function sync(){
  ...
  $("pMobile").textContent=d.mobile; $("pCoach").textContent=d.coach;
  $("pName").textContent=d.customerName; $("pAddress").textContent=d.address; $("pPhone").textContent=d.phone;
  ...
  for(let i=0;i<Math.max(5,d.items.length);i++){
    const x=d.items[i], tr=document.createElement("tr");
    tr.innerHTML=`<td>${x?i+1:""}</td><td>${x?esc(x.description):""}</td><td>${x?x.qty:""}</td><td>${x?money(x.rate):""}</td><td>${x?"₹ "+money(x.qty*x.rate):""}</td>`;
    t.appendChild(tr);
  }
}
